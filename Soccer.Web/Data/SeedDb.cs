...
namespace Soccer.Web.Data
{
    public class SeedDb
    {
            ...
            public async Task SeedAsync()
            {
                ...
                var user1 = await CheckUserAsync("2020", "Juan", "Zuluaga", "jzuluaga55@hotmail.com", "350 634 2747", "Calle Luna Calle Sol", UserType.User);
                var user2 = await CheckUserAsync("3030", "Juan", "Zuluaga", "carlos.zuluaga@globant.com", "350 634 2747", "Calle Luna Calle Sol", UserType.User);
                var user3 = await CheckUserAsync("4040", "Juan", "Zuluaga", "juanzuluaga2480@correo.itm.edu.co", "350 634 2747", "Calle Luna Calle Sol", UserType.User);
                await CheckPreditionsAsync(user1, user2, user3);
            }

            private async Task CheckPreditionsAsync(UserEntity user1, UserEntity user2, UserEntity user3)
            {
                if (!_context.Predictions.Any())
                {
                    await AddPredictionsAsync(user1);
                    await AddPredictionsAsync(user2);
                    await AddPredictionsAsync(user3);
                }
            }

            private async Task AddPredictionsAsync(UserEntity user)
            {
